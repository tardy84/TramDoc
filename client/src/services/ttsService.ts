...
async function synthesizeGoogle(text: string, voice: string, signal?: AbortSignal): Promise<Blob> {
    ...
    try {
        ...
        console.warn(`[GoogleTTS Client] Lỗi API, tự động chuyển qua Azure.`, e);
        return synthesizeAzure(text, 'azure-vi-VN-HoaiMyNeural');
    }
}
...
async function synthesizeGemini(text: string, voice: string, signal?: AbortSignal): Promise<Blob> {
    ...
    try {
        ...
        if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
            throw new Error('Gemini TTS: API trả về không có audio data');
        }
        ...
        for (const p of parts) {
            if (p.inlineData && p.inlineData.mimeType.startsWith('audio/')) {
                ...
                // Wrap raw PCM in WAV header (L16, mono, 24000Hz)
        ...
        console.warn(`[GeminiTTS Client] Lỗi API, tự động chuyển qua Google Cloud TTS.`, e);
